...
const TalkHead = () => {
	...
	const [loadcontactheader, setloadcontactheader] = useState(true)
	useEffect(() => {
		fetch(`${URL}/contact-header`)
			.then((res) => res.json())
			.then((contactheader) => {
				setcontactheader(contactheader)
				setloadcontactheader(false)
			})
	}, [])

	if (loadcontactheader) {
		...
		<div className={styles.back} style={{ backgroundImage: "url('/Talktous_NEW.jpg')", backgroundPosition: 'center top 55%', backgroundSize: '100%' }}>
			<div className={styles.text}>
				...
				<div style={{ marginTop: '2rem' }}>
					<p dangerouslySetInnerHTML={{__html: contactheader.data.attributes.subHead}}></p>
				</div>
			</div>
			<div style={{ margin: '0' }} className={styles.aiLogo}>
				<svg version='1.1' id='Isolation_Mode' xmlns='http://www.w3.org/2000/svg' xlink='http://www.w3.org/1999/xlink' x='0px' y='0px' viewBox='0 0 97.1 66.1' enableBackground='new 0 0 97.1 66.1' space='preserve' fill='#fff' width='24rem' style={{ marginBottom: '-1.04rem', marginRight: '-0.8rem' }}>
					<path
