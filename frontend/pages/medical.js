...
const medical = () => {
  const [medicaltext, setmedicaltext] = useState(null);
  const [loadmedicaltext, setloadmedicaltext] = useState(true);
  useEffect(() => {
    fetch(`${URL}/medical`)
      .then((res) => res.json())
      .then((medicaltext) => {
        setmedicaltext(medicaltext);
        setloadmedicaltext(false);
      });
  }, []);

  const [medicaldrops, setmedicaldrops] = useState(null);
  const [loadmedicaldrops, setloadmedicaldrops] = useState(true);
  useEffect(() => {
    fetch(`${URL}/medical-drops?sort=id&populate=*`)
      .then((res) => res.json())
      .then((medicaldrops) => {
        setmedicaldrops(medicaldrops);
        setloadmedicaldrops(false);
      });
  }, []);
      ...
      <div className={styles.main}>
        <div className={styles.sec1}>
          <svg
            ...
            x="0px"
            y="0px"
            viewBox="0 0 91.6 87.1"
            enableBackground="new 0 0 91.6 87.1"
            space="preserve"
            fill="#002d62"
            width="12rem"
          >
